...
- **Browser Origin policy (WS upgrade + REST CORS).** One allowlist covers both: localhost,
  the same host the request was dialed on, anything in `CC_WS_ALLOWED_ORIGINS`, and the `null`
  origin a console opened as a `file://` page sends (on by default; `CC_ALLOW_FILE_ORIGIN=0`
  withdraws it). An allowed origin is reflected in `Access-Control-Allow-Origin` with no
  credentials flag — the bus has no cookies, so the grant unlocks nothing without the bearer
  token. Any other origin gets no grant and its WS upgrade is refused; non-browser Node clients
